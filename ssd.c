...
int parse_user_args(int argc, char *argv[], struct user_args* uargs) {
    char **positionals;
    int raidtype = -1;
    int ndisk = 0, diskid = 0, crypt_scheme=0;
    ...
    static struct option long_options[] = {
        ...
        {"crypto", required_argument, 0, 'c'},  //added for term paper
        {0, 0, 0, 0}
    };
    ...
    while ((opt = getopt_long(argc, argv,"05n:", long_options, &long_index )) != -1) {
        switch (opt) {
            ...
            case 'p':
                ...
                break;
            case 'c':                   //added for term paper
                crypt_scheme = atoi(optarg);
                if (crypt_scheme < 0) {
                    printf("Error! wrong crypt_scheme, it must be >= 0, but get %d!\n", crypt_scheme);
                    return -1;
                }
                uargs->crypto = crypt_scheme;
                break;
...
//added for term paper
    // Assign all var related to aes128
    if (uargs->crypto)
    {
        ssd->crypto=uargs->crypto;
    }
...
{   
    ...
    if((complete_flag == 1)&&(new_request->subs==NULL))               
    {
       ...
       // new_request->response_time=ssd->current_time+1000;     
        new_request->response_time=ssd->current_time+1000+39500*1000;   
    ...
    if(req != NULL)
    {
        if(req->distri_flag == 0)
        {
            ...
            else
            {
                ...
                //req->response_time=ssd->current_time+1000+107500*1000*req->size;   
               //req->response_time=ssd->current_time+1000+39500*1000*req->size;   
            }
...
void trace_output(struct ssd_info* ssd){
    ...
    while(req != NULL)	
    {
        ...
        else
        {
            ...
            {		
                if (ssd->crypto == 1) {
                    end_time=end_time+1000+39500*1000*req->size; 
                    printf("\nreq->response_time1=%16lld\n", req->response_time);

                }  
                if(ssd->crypto == 2){
                    end_time=end_time+1000+107500*1000*req->size;   
                    printf("\nreq->response_time2=%16lld\n", req->response_time);

                }

                req->response_time = end_time;
                latency = end_time-req->time;
                //printf("\nreq->response_time=%16lld\n", req->response_time);
